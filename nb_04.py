...
def get_model(data, lr=0.5, nh=50):
    for i in data.train_ds.take(1): m = i[0].shape[1]
    model = tf.keras.models.Sequential([layers.Dense(nh), layers.ReLU(), layers.Dense(data.c)])
    model.build(input_shape=(None, m))
    return model, optimizers.SGD(learning_rate=lr)
...
class Runner():
    ...
    def all_batches(self, dl):
        self.iters = tf.data.experimental.cardinality(dl).numpy()
        for xb, yb in dl:
            if self.stop: break
